...
_lastNDateBtns.forEach(t=>{t.addEventListener("click",()=>{_lastNDateBtns.forEach(t=>t.classList.remove("active")),t.classList.add("active"),lastNGamesCount=parseInt(t.dataset.n)})}),initPlayerSearch("lastNSearch","lastNSearchResults",(t,e)=>{lastNSelectedPlayer={id:t,name:e},document.getElementById("lastNPlayerChip").innerHTML=`\n                <div class="player-chip">\n                    <span>${e}</span>\n                    <button class="player-chip-remove" aria-label="Remove player" onclick="lastNSelectedPlayer=null; document.getElementById('lastNPlayerChip').innerHTML=''; document.getElementById('lastNBtn').disabled=true;">&times;</button>\n                </div>\n            `,document.getElementById("lastNBtn").disabled=!1},"lastNBtn");let playoffsData=null,activeConference="east";async function loadPlayoffs(force=false){if(playoffsData&&!force){showConference(activeConference);return}const t=document.getElementById("playoffsContent");t.innerHTML='<div class="loading"><div class="spinner"></div> Loading bracket...</div>';try{const t=await _fetchWithAbort("playoffs","/api/playoffs");playoffsData=await t.json(),showConference(activeConference)}catch(e){t.innerHTML=`<div class="empty-state"><div class="empty-state-icon">&#9888;</div><div class="empty-state-title">Error Loading Bracket</div><p>${esc(e.message)}</p></div>`}}function showConference(t){if(!playoffsData)return;activeConference=t,document.getElementById("playoffEastBtn").classList.toggle("active","east"===t),document.getElementById("playoffWestBtn").classList.toggle("active","west"===t);const e=document.getElementById("playoffsContent");e.innerHTML="";const piA="east"===t?playoffsData.playinActual&&playoffsData.playinActual.east||{}:playoffsData.playinActual&&playoffsData.playinActual.west||{};drawBracket("east"===t?playoffsData.east:playoffsData.west,"east"===t?"Eastern Conference":"Western Conference",e,piA,playoffsData.seriesResults||{})}function drawBracket(t,e,s,piActual,seriesResults){const a=220,n=38,i=20,l=16,piA=piActual||{},seed7=piA.seed7TeamId,g3w=piA.g3WinnerTeamId;const ts=[...t].sort((a,b)=>a.rank-b.rank);const s7=seed7?t.find(x=>x.teamId===seed7):{rank:7,name:"Game 1 Winner",wins:"",losses:"",teamId:null};const s8=g3w?t.find(x=>x.teamId===g3w):{rank:8,name:"Game 3 Winner",wins:"",losses:"",teamId:null};let r=[ts[0],s8,ts[3],ts[4],ts[2],ts[5],ts[1],s7];const o=t.slice(6,10),d=788,c=1.44,m=window.devicePixelRatio||1,p=document.createElement("canvas");p.width=1134.72*m,p.height=778*m,p.style.cssText="max-width:100%; display:block; margin-bottom:24px; width:100%;";const v=p.getContext("2d");v.scale(c*m,c*m);const h="#1a1a24",y="#2a2a3a",f="#ffffff",g="#a0a0b0",u="#ff6b35",b="#f59e0b",T="#3a3a4a";const _sr=seriesResults||{};const serMap={};[[0,1],[2,3],[4,5],[6,7]].forEach(([ai,bi])=>{const tA=r[ai],tB=r[bi];if(!tA||!tB||!tA.teamId||!tB.teamId)return;const k=`${Math.min(tA.teamId,tB.teamId)}_${Math.max(tA.teamId,tB.teamId)}`;const sd=_sr[k];if(sd){serMap[tA.teamId]={w:sd[String(tA.teamId)]||0,l:sd[String(tB.teamId)]||0};serMap[tB.teamId]={w:sd[String(tB.teamId)]||0,l:sd[String(tA.teamId)]||0}}});function $(t,e,s,i){v.fillStyle=i?"#22222e":h,x(v,t,e,a,n,5),v.fill(),v.strokeStyle=i?u:y,v.lineWidth=i?1.5:1,x(v,t,e,a,n,5),v.stroke();const dispRank=s.rank;const rankColor=dispRank<=6?"#4ade80":b;v.fillStyle=rankColor+"22",x(v,t+6,e+8,22,22,3),v.fill(),v.font="bold 10px Inter, sans-serif",v.fillStyle=rankColor,v.textAlign="center",v.fillText(dispRank,t+17,e+23),v.font="500 11px Inter, sans-serif",v.fillStyle=f,v.textAlign="left";let r=s.name;for(v.font="11px Inter, sans-serif";v.measureText(r).width>140&&r.length>4;)r=r.slice(0,-1);r!==s.name&&(r+="…"),v.fillText(r,t+34,e+23),v.font="10px Inter, sans-serif",v.fillStyle=g,v.textAlign="right";const _sd=serMap[s.teamId];v.fillText(_sd?`${_sd.w}-${_sd.l}`:`${s.wins}-${s.losses}`,t+a-6,e+23)}function x(t,e,s,a,n,i){t.beginPath(),t.moveTo(e+i,s),t.lineTo(e+a-i,s),t.quadraticCurveTo(e+a,s,e+a,s+i),t.lineTo(e+a,s+n-i),t.quadraticCurveTo(e+a,s+n,e+a-i,s+n),t.lineTo(e+i,s+n),t.quadraticCurveTo(e,s+n,e,s+n-i),t.lineTo(e,s+i),t.quadraticCurveTo(e,s,e+i,s),t.closePath()}function L(t){return i+46*t}v.fillStyle="#12121a",v.fillRect(0,0,d,540);for(let t=0;t<8;t++){const e=r[t];if(!e)continue;const s=L(t);$(l,s,e,!1)}const w=284,A=552;v.strokeStyle=T,v.lineWidth=1.5;const k=[];[[0,1],[2,3],[4,5],[6,7]].forEach(([t,e],s)=>{const a=L(t)+19,n=L(e)+19,i=260,l=(a+n)/2;v.beginPath(),v.moveTo(236,a),v.lineTo(i,a),v.stroke(),v.beginPath(),v.moveTo(236,n),v.lineTo(i,n),v.stroke(),v.beginPath(),v.moveTo(i,a),v.lineTo(i,n),v.stroke(),v.beginPath(),v.moveTo(i,l),v.lineTo(w,l),v.stroke(),k.push(l-19)}),k.forEach((t,e)=>{v.fillStyle=h,x(v,w,t,a,n,5),v.fill(),v.strokeStyle=y,v.lineWidth=1,x(v,w,t,a,n,5),v.stroke(),v.font="10px Inter, sans-serif",v.fillStyle=g,v.textAlign="center",v.fillText("Semifinal",394,t+19+4)});const S=[];[[0,1],[2,3]].forEach(([t,e],s)=>{const a=k[t]+19,n=k[e]+19,i=528,l=(a+n)/2;v.strokeStyle=T,v.lineWidth=1.5,v.beginPath(),v.moveTo(504,a),v.lineTo(i,a),v.stroke(),v.beginPath(),v.moveTo(504,n),v.lineTo(i,n),v.stroke(),v.beginPath(),v.moveTo(i,a),v.lineTo(i,n),v.stroke(),v.beginPath(),v.moveTo(i,l),v.lineTo(A,l),v.stroke(),S.push(l-19)}),S.forEach(t=>{v.fillStyle=h,x(v,A,t,a,n,5),v.fill(),v.strokeStyle=u,v.lineWidth=1,x(v,A,t,a,n,5),v.stroke(),v.font="bold 10px Inter, sans-serif",v.fillStyle=u,v.textAlign="center",v.fillText("Conf. Finals",662,t+19+4)}),v.font="bold 10px Inter, sans-serif",v.fillStyle=g,v.textAlign="center",v.fillText("FIRST ROUND",126,8),v.fillText("SEMIFINALS",394,8),v.fillText("CONF. FINALS",662,8);v.font="bold 10px Inter, sans-serif",v.fillStyle=b,v.textAlign="left",v.fillText("PLAY-IN TOURNAMENT (Seeds 7-10)",l,414);const E=220,piY=436,piRowH=42,piGap=28;function drawPiTeam(tx,ty,team,highlight){v.fillStyle=highlight?"#1e1e2e":h,x(v,tx,ty,E,n,5),v.fill(),v.strokeStyle=highlight?b:y,v.lineWidth=highlight?1.5:1,x(v,tx,ty,E,n,5),v.stroke();const rankCol=b;v.fillStyle=rankCol+"22",x(v,tx+6,ty+8,22,22,3),v.fill(),v.font="bold 10px Inter, sans-serif",v.fillStyle=rankCol,v.textAlign="center",v.fillText((piA.initialSeeds&&piA.initialSeeds[String(team.teamId)])||team.rank,tx+17,ty+23),v.font="10px Inter, sans-serif",v.fillStyle=f,v.textAlign="left";let nm=team.name;for(;v.measureText(nm).width>140&&nm.length>4;)nm=nm.slice(0,-1);nm!==team.name&&(nm+="…"),v.fillText(nm,tx+34,ty+23),v.fillStyle=g,v.textAlign="right",v.fillText(team.gameScore!=null?String(team.gameScore):(!team.losses||team.losses==="?"?"—":team.wins+"-"+team.losses),tx+E-6,ty+23)}function drawPiMatchup(mx,my,topTeam,botTeam,label,result,topWin,botWin){v.font="bold 9px Inter, sans-serif",v.fillStyle=g,v.textAlign="left",v.fillText(label,mx,my-4),drawPiTeam(mx,my,topTeam,!!topWin),drawPiTeam(mx,my+piRowH,botTeam,!!botWin),v.font="9px Inter, sans-serif",v.fillStyle=b,v.textAlign="left",v.fillText(result,mx,my+piRowH+n+8)}const piCol1=l,piCol2=l+E+piGap,piCol3=l+(E+piGap)*2;const scrs=piA.gameScores||{};function piKey(a,b){return a.teamId<b.teamId?a.teamId+"_"+b.teamId:b.teamId+"_"+a.teamId}function sT(t,sc){if(!sc||sc[String(t.teamId)]==null)return t;const r=Object.assign({},t);r.gameScore=sc[String(t.teamId)];return r}const g1Sc=o[0]&&o[1]?scrs[piKey(o[0],o[1])]:null;const g2Sc=o[2]&&o[3]?scrs[piKey(o[2],o[3])]:null;const g1WinIdx=g1Sc?g1Sc[String(o[0].teamId)]>=g1Sc[String(o[1].teamId)]?0:1:-1;const g1Loser=g1WinIdx>=0?o[1-g1WinIdx]:null;const g2WinIdx=g2Sc?g2Sc[String(o[2].teamId)]>=g2Sc[String(o[3].teamId)]?0:1:-1;const g2Winner=g2WinIdx>=0?o[2+g2WinIdx]:null;const g3Top=piA.g1LoserTeamId&&o[0]?o[0].teamId===piA.g1LoserTeamId?o[0]:o[1]&&o[1].teamId===piA.g1LoserTeamId?o[1]:null:{rank:"?",name:"G1 Loser",wins:"?",losses:"?"};const g3Bot=piA.g2WinnerTeamId&&o[2]?o[2].teamId===piA.g2WinnerTeamId?o[2]:o[3]&&o[3].teamId===piA.g2WinnerTeamId?o[3]:null:{rank:"?",name:"G2 Winner",wins:"?",losses:"?"};const g3Sc=g3Top.teamId&&g3Bot.teamId?scrs[piKey(g3Top,g3Bot)]:null;const g3WTop=!!(piA.g3WinnerTeamId&&g3Top.teamId&&g3Top.teamId===piA.g3WinnerTeamId);const g3WBot=!!(piA.g3WinnerTeamId&&g3Bot.teamId&&g3Bot.teamId===piA.g3WinnerTeamId);drawPiMatchup(piCol1,piY,sT(o[0],g1Sc),sT(o[1],g1Sc),"GAME 1","→ Winner clinches 7th seed",g1WinIdx===0,g1WinIdx===1),drawPiMatchup(piCol2,piY,sT(o[2],g2Sc),sT(o[3],g2Sc),"GAME 2","→ Winner advances · Loser eliminated",g2WinIdx===0,g2WinIdx===1),drawPiMatchup(piCol3,piY,sT(g3Top,g3Sc),sT(g3Bot,g3Sc),"GAME 3","→ Winner clinches 8th seed · Loser eliminated",g3WTop,g3WBot),s.appendChild(p)}let _tradesData=null,_tradesMonth="";const _TYPE_STYLE={Trade:"background:#3b82f6;color:#fff",Signing:"background:#4ade80;color:#000",Waive:"background:#ef4444;color:#fff",ContractConverted:"background:#f59e0b;color:#000"};const _TYPE_LABEL={Trade:"TRADE",Signing:"SIGN",Waive:"WAIVE",ContractConverted:"CONVERT"};function _buildTradesMonthBtns(){const sel=document.getElementById("tradesMonthSelector");const now=new Date();const btns=[];for(let i=0;i<6;i++){const d=new Date(now.getFullYear(),now.getMonth()-i,1);const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;const label=d.toLocaleDateString("en-US",{month:"short",year:"numeric"});btns.push({key,label})}sel.innerHTML=btns.map((b,i)=>`<button class="date-btn${i===0?" active":""}" data-month="${b.key}">${b.label}</button>`).join("");_tradesMonth=btns[0].key;sel.querySelectorAll(".date-btn").forEach(btn=>{btn.addEventListener("click",()=>{sel.querySelectorAll(".date-btn").forEach(b=>b.classList.remove("active"));btn.classList.add("active");_tradesMonth=btn.dataset.month;_renderTrades()})})}async function loadTrades(force=false){if(_tradesData&&!force){_renderTrades();return}const el=document.getElementById("tradesContent");el.innerHTML='<div class="loading"><div class="spinner"></div> Loading transactions...</div>';try{const r=await _fetchWithAbort("trades","/api/trades"),d=await r.json();if(!r.ok)throw new Error(d.detail||"Failed to load trades");_tradesData=d;_renderTrades()}catch(e){el.innerHTML=`<div class="empty-state"><div class="empty-state-icon">&#9888;</div><div class="empty-state-title">Error Loading Trades</div><p>${esc(e.message)}</p></div>`}}const _HL_PATTERNS=[[/\b10-Day Contract/gi,"#f59e0b"],[/\bRest-of-Season Contract/gi,"#4ade80"],[/\bTwo-Way Contract/gi,"#a78bfa"],[/\bExhibit 10 Contract/gi,"#94a3b8"],[/\b\d+-Year Contract/gi,"#ff6b35"],[/\bMinimum Contract/gi,"#60a5fa"],[/\bQualifying Offer/gi,"#60a5fa"]];_buildTradesMonthBtns();let _seasonDoublesData=null;async function loadSeasonDoubles(force=false){if(_seasonDoublesData&&!force){renderSeasonDoubles();return}const el=document.getElementById("seasonDoublesContent");el.innerHTML='<div class="loading"><div class="spinner"></div> Loading season leaders...</div>';try{const r=await _fetchWithAbort("seasonDoubles","/api/season/doubles"),d=await r.json();if(!r.ok)throw new Error(d.detail||"Failed to load");_seasonDoublesData=d;renderSeasonDoubles()}catch(e){el.innerHTML=`<div class="empty-state"><div class="empty-state-icon">&#9888;</div><div class="empty-state-title">Error Loading Data</div><p>${esc(e.message)}</p></div>`}}function renderSeasonDoubles(){const el=document.getElementById("seasonDoublesContent");const d=_seasonDoublesData;if(!d)return;const tbl=(title,list,isTd)=>{if(!list||list.length===0)return`<div class="card" style="padding:20px;"><h3 style="margin-bottom:10px;color:var(--accent);">${title}</h3><p style="color:var(--text-secondary);">No data available</p></div>`;return`<div class="card" style="overflow-x:auto;"><h3 style="padding:15px 20px;background:var(--bg-secondary);margin:0;border-bottom:1px solid var(--border);">${title}</h3><table class="doubles-table"><thead><tr><th>Rank</th><th style="text-align:left;">Player</th><th>Team</th><th>Count</th>${isTd?'<th>Details</th>':''}</tr></thead><tbody>${list.map(p=>`<tr id="td-row-${p.playerId}"><td>${p.rank}</td><td style="text-align:left;font-weight:500;">${p.name}</td><td>${p.team}</td><td class="highlight">${p.count}</td>${isTd?`<td><button class="refresh-btn" style="font-size:0.75rem;padding:2px 8px;" onclick="toggleTdGames(${p.playerId},this)">Details</button></td>`:''}</tr>${isTd?`<tr id="td-details-${p.playerId}" style="display:none;"><td colspan="5"><div id="td-games-${p.playerId}" style="padding:8px;"></div></td></tr>`:''}`).join('')}</tbody></table></div>`};el.innerHTML=`<div class="season-doubles-grid">${tbl("Double-Doubles",d.doubleDoubles?d.doubleDoubles.slice(0,20):[],false)}${tbl("Triple-Doubles",d.tripleDoubles?d.tripleDoubles.slice(0,20):[],true)}</div>`}async function toggleTdGames(playerId,btn){const detailRow=document.getElementById(`td-details-${playerId}`);if(detailRow.style.display!=="none"){detailRow.style.display="none";btn.textContent="Details";return}detailRow.style.display="table-row";const container=document.getElementById(`td-games-${playerId}`);container.innerHTML='<div class="loading" style="padding:10px;"><div class="spinner"></div> Loading games...</div>';btn.textContent="Hide";try{const r=await _fetchWithAbort("tdGames_"+playerId,`/api/season/triple-double-games/${playerId}`),d=await r.json();if(!d.games||d.games.length===0){container.innerHTML='<p style="color:var(--text-secondary);font-size:0.85rem;">No triple-double games found</p>';return}container.innerHTML=`<div style="overflow-x:auto;"><table class="boxscore-table" style="font-size:0.72rem;"><thead><tr><th>Date</th><th>Matchup</th><th>PTS</th><th>REB</th><th>AST</th><th>STL</th><th>BLK</th></tr></thead><tbody>${d.games.map(g=>`<tr><td style="white-space:nowrap;">${g.date}</td><td style="white-space:nowrap;">${g.matchup}</td><td class="${statClass('points',g.points)}">${g.points}</td><td class="${statClass('rebounds',g.rebounds)}">${g.rebounds}</td><td class="${statClass('assists',g.assists)}">${g.assists}</td><td class="${statClass('steals',g.steals)}">${g.steals}</td><td class="${statClass('blocks',g.blocks)}">${g.blocks}</td></tr>`).join('')}</tbody></table></div>`}catch(e){container.innerHTML='<p style="color:#ef4444;">Error loading games</p>'}}let _seasonHighsData=null;
