...
def test_game_players_skips_zero_minutes(client, monkeypatch):
    """Players with empty minutes are excluded from boxscore details."""
    from conftest import GAME_ID, _V3_PLAYER_HEADERS, _V3_TEAM_HEADERS, TEAM_ID_LAL
    ...
    mock_bs = MagicMock()
    mock_bs.player_stats.get_dict.return_value = {
        "headers": _V3_PLAYER_HEADERS,
        "data": player_rows,
    }
    mock_bs.team_stats.get_dict.return_value = {
        "headers": _V3_TEAM_HEADERS,
        "data": team_rows,
    }
    monkeypatch.setattr(
        "routes.players.get_cached_boxscore_v3", lambda *a, **kw: mock_bs
    )
    cache.clear()
    r = client.get(f"/api/games/{GAME_ID}/players")
    assert r.status_code == 200
    lal = r.json()["teams"][0]
    assert len(lal["players"]) == 1
    assert lal["players"][0]["name"] == "LeBron James"
    cache.clear()
